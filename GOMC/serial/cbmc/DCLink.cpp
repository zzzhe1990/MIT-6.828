...
namespace cbmc
{
   ...
   void DCLink::BuildNew(TrialMol& newMol, uint molIndex)
   {
      ...
      {
#ifdef DEBUG
//	  printf("atom new position of trail: %d is (%lf, %lf, %lf)\n", trial, positions[trial].x, positions[trial].y, positions[trial].z);
#endif
         ljWeights[trial] = GenerateDihedrals(angles, angleEnergy,
					      angleWeights);
         uint winner = prng.PickWeighted(angleWeights, data->nDihTrials,
	 ...
	 if(ljWeights[trial] < 10e-200)
      ...
      if ( ( WinEnergy * data->ff.beta ) > (2.3*200.00) || WinEnergy * data->ff.beta < -2.303e308){
      	dihLJWeight = 0.0;
      	inter[winner] = 0.0;
      	real[winner] = 0.0;
      	self[winner] = 0.0;
